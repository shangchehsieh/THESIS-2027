...
def split_dataset2020(data_root, ratio_1=0.20, seed=42):
    patients_dir = glob.glob(os.path.join(data_root, "BraTS20*"))
    n_patients = len(patients_dir)
    print(f"total patients: {n_patients}")
    pid_idx = np.arange(n_patients)
    ...
    split_idx = math.ceil(n_patients * ratio_1)
    # split_idx = int(n_patients * ratio_1)
    val_patients_list = patients_dir[:split_idx]
    train_patients_list = patients_dir[split_idx:]
