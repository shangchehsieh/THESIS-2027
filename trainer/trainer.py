...
class Trainer(object):
    ...
    def training(self, epoch):
        ...
        for i, sample in enumerate(tbar):
            ...
            if self.args.cuda:
                image = image.cuda()
                target = target.cuda()
            # LR_Scheduler.__call__ is (optimizer, i, epoch, best_pred). The upstream
            # code passed (epoch, i), so T = i*iters_per_epoch + epoch never exceeded
            # ~8% of N and the poly schedule effectively never decayed.
    ...
    def _test(self, drop=[], epoch=0):
        ...
        for i, sample in enumerate(tbar):
            ...
            with torch.no_grad():
                # output, df = self.model(image, channel=drop)
    ...
    def forward_batch(self, image, target):
        output, df = self.model(image)
        missing_logits = torch.stack([df[l] for l in range(4)], dim=0)
        df_full = torch.mean(missing_logits, dim=0)

        loss = self.criterion(output, target, df, df_full)
